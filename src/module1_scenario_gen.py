...
# パターン定義JSONのパス
PATTERNS_JSON_PATH = Path(__file__).resolve().parent.parent / "data" / "plot_twist_patterns.json"

# デフォルトのキャラクター服装（新規エピソード作成時に使用）
DEFAULT_CHARACTER_OUTFITS = {
    "はな": "white and blue horizontal striped T-shirt, light blue jeans, long straight brown hair",
    "さき": "SHORT BOB HAIRCUT (chin-length, NOT long hair), FOREHEAD VISIBLE (no bangs, hair parted to side), pink/salmon cardigan over white top, brown chino pants",
    "なおみ": "ROUND GLASSES (MUST wear round glasses), LONG WAVY BROWN HAIR (shoulder-length or longer), white collared blouse, navy blue skirt",
    "まさと": "casual shirt, jeans, short black hair",
    "ともや": "hoodie, casual pants, medium brown hair",
    "ようた": "polo shirt, chinos, short brown hair",
}

# PlotTwistパターン（JSONから読み込み）
_plot_twist_data = None

def load_plot_twist_patterns() -> dict:
    """plot_twist_patterns.jsonを読み込む"""
    global _plot_twist_data
    if _plot_twist_data is None:
        with open(PATTERNS_JSON_PATH, "r", encoding="utf-8") as f:
            _plot_twist_data = json.load(f)
    return _plot_twist_data

def get_pattern_names() -> dict[str, list[str]]:
    """カテゴリごとのパターン名リストを取得"""
    data = load_plot_twist_patterns()
    return {
        category: [p["name"] for p in info["patterns"]]
        for category, info in data.items()
    }

def get_pattern_details(pattern_name: str) -> dict | None:
    """パターン名から詳細情報を取得"""
    data = load_plot_twist_patterns()
    for category, info in data.items():
...
def select_random_pattern() -> tuple[str, str]:
    ...
    category = random.choice(list(pattern_names.keys()))
    pattern = random.choice(pattern_names[category])
...
{{
  ...
  "panels": [
    {{
      ...
      "characters": ["登場キャラクター"],
      "dialogue": [
        {{"character": "キャラ名", "text": "セリフ", "type": "normal/thought/shout/effect_text"}}
      ],
      "background": "背景の説明",
      "effects": ["効果"]
    }},
...
def add_character_outfits(scenario: dict) -> None:
    """シナリオに登場するキャラクターの服装を自動追加"""
    # 既にcharacter_outfitsがある場合は何もしない
    if "character_outfits" in scenario:
        return
    ...
    for char in characters:
        if char in DEFAULT_CHARACTER_OUTFITS:
            outfits[char] = DEFAULT_CHARACTER_OUTFITS[char]
        else:
            # デフォルトにないキャラクターは汎用服装
            outfits[char] = "casual outfit"

    # シナリオに追加
    if outfits:
        scenario["character_outfits"] = outfits
        print(f"[Module1] キャラクター服装を自動追加: {list(outfits.keys())}")
...
def main():
    ...
    # キャラクター服装を自動追加
    add_character_outfits(scenario)
