...
class PreflightChecker:
    ...
    def check_bubble_types(self, scenario: dict) -> None:
        """吹き出しタイプのチェック（許可: normal, thought, shout, effect_text）"""
        ALLOWED_TYPES = ["normal", "thought", "shout", "effect_text"]
        for panel in scenario.get("panels", []):
            for dialogue in panel.get("dialogue", []):
                bubble_type = dialogue.get("type", dialogue.get("bubble_type", "normal"))
                if bubble_type not in ALLOWED_TYPES:
                    self.errors.append(
                        f"パネル{panel.get('panel_number', panel.get('number'))}で不正なtype: {bubble_type}（許可: {', '.join(ALLOWED_TYPES)}）"
                    )

    def check_shout_in_panel4(self, scenario: dict) -> None:
        """4コマ目にshout（ツッコミ）があるかチェック"""
        panels = scenario.get("panels", [])
        if len(panels) >= 4:
            panel4 = panels[3]
            has_shout = any(
                d.get("type", d.get("bubble_type")) == "shout" 
                for d in panel4.get("dialogue", [])
            )
            if not has_shout:
                self.warnings.append("4コマ目にshout（ツッコミ）タイプの吹き出しがありません")
    ...
    def run_all_checks(self, scenario: dict) -> dict:
        ...
        self.check_shout_in_panel4(scenario)
...
def main():
    ...
    parser.add_argument("--episode", required=True, help="入力エピソードJSONファイル")
    ...
    print(f"[Module2] 検証済みエピソード出力: {output_path}")
