...
- CONSISTENT BRIGHTNESS: Use medium brightness levels - avoid too dark or too bright images
- CONSISTENT COLOR TEMPERATURE: Use warm but balanced lighting - maintain consistent color palette across all panels
...
"""
        # effects配列の処理を追加
        if effects:
            prompt += "\nMANGA EFFECTS (draw as stylized manga sound effects / emotion text directly on the image):\n"
            for effect in effects:
                if effect and effect.strip():  # 空でない効果のみ処理
                    prompt += f"- Draw '{effect}' as a large manga-style effect text in the scene\n"
        ...
        # カット割（inset cuts）の処理
        inset_cuts = panel.get("inset_cuts", [])
        if inset_cuts:
            prompt += "\nINSET CUTS (manga-style diagonal cut panel):\n"
            for cut in inset_cuts:
                position = cut.get("position", "bottom_left")
                shape = cut.get("shape", "diagonal_triangle")
                size = cut.get("size", "small")
                character = cut.get("character", "")
                expression = cut.get("expression", "")
                description = cut.get("description", "")
                text = cut.get("text", "")
                
                prompt += f"- Create a diagonal triangle cut in the {position} corner\n"
                prompt += f"- Draw a DIAGONAL LINE (border) from top-left to bottom-right of the panel\n"
                prompt += f"- The diagonal line should be a thin dark brown/black border line\n"
                prompt += f"- NO white space or gap - the inset cut fills the entire triangle area\n"
                prompt += f"- Inside the triangle area (below the diagonal line): {description}\n"
                prompt += f"- Character: {character} with {expression} expression (close-up face)\n"
                if text:
                    prompt += f"- Text near face: '{text}' (small, handwritten Japanese style)\n"
                prompt += f"- The main scene continues in the upper triangle (above the diagonal line)\n"
                prompt += f"- ONLY the diagonal line is visible as a border - no other borders\n"
